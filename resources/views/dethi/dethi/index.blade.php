...
@section('content')
            ...
            <div class="card card-custom">
                <div class="card-header card-header-tabs-line">
                    <div class="card-title">
                        <h3 class="card-label text-uppercase">Danh sách đề thi</h3>
                    </div>
                        ...
                        <tbody>
                            @foreach ($model as $key => $dt)
                                <tr class="text-center">
                                    ...
                                    <td name="made" style="width: 2%">{{ $dt->made }}</td>
                                    <td name='tende' class="text-left" style="width: 20%">{{ $dt->tende }}</td>
                                    <td name='soluongcauhoi' style="width: 2%">{{$dt->socauhoi}}</td>
                                    <td class="text-center" style="width:8%">
                                        <a href="{{'/DeThi/ChiTiet/'.$dt->made}}" title="Chi tiết"
                                            class="btn btn-sm btn-clean btn-icon">
                                             <i class="icon-lg la la-th-list text-primary "></i>
                                         </a>
                                        <button title="Sửa thông tin"
                                            onclick="edit(this,'{{ $dt->made }}')"
                                            data-target="#edit" data-toggle="modal" class="btn btn-sm btn-clean btn-icon">
                                            <i class="icon-lg la flaticon-edit-1 text-primary "></i>
                                        </button>

                                        <button title="Xóa thông tin" type="button"
                                            onclick="cfDel('{{ '/DeThi/delete/' . $dt->made }}')"
    ...
    <div id="themmoi" tabindex="-1" role="dialog" aria-hidden="true" class="modal fade kt_select2_modal">
        <form action="{{ '/DeThi/store' }}" method="POST" id="frm_dethi" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="modal-body">
                        <div class="form-group row">
                            <div class="col-md-12">
                                ...
                                <input type="text" name="tende" class="form-control" required>
                            </div>
                        </div>
                    </div>
